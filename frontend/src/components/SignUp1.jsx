...
const SignUp = () => {
  ...
  return (
    <>
    <Top/>
    <section className='h-screen  bg-violet-100 bg-cover bg-no-repeat bg-center flex justify-end items-center  ' style={{backgroundImage:"url(/SignUp.png)"}}>
            <div className='absolute w-130 text-gray-600 bg-violet-200  flex flex-col items-center m-15 p-4 rounded-lg shadow-lg'>
                ...
                <h1 className='text-xs'>Already have an account?<Link to="/SignIn" className="text-blue-600">SignIn here</Link></h1>

                <form  className='flex flex-col w-full items-center mt-8' onSubmit={handleSubmit(onSubmit)}>
                    <input className="hover:shadow-lg transition duration-200 outline-none rounded-lg cursor-pointer p-2 py-3 bg-violet-100  w-[80%] " type="text" 
                    ...
                    <input className="hover:shadow-lg transition duration-200 outline-none rounded-lg cursor-pointer p-2 py-3 bg-violet-100 w-[80%]" type="text" placeholder='College ID*'
                    {...register('collegeid',({
                        required:true,
                        minLength:{value:8,message:"min length is 8"}
                    }))} />
                    <div className='h-5 text-red-700 text-sm'>{errors.collegeid&&<p className='text-xm'>*{errors.collegeid.message}</p>}</div>
                    <input className="hover:shadow-lg transition duration-200 outline-none rounded-lg cursor-pointer p-2 py-3 bg-violet-100 w-[80%]" type="text" placeholder='Phone Number*'
                    {...register("phonenumber",{
                        required:true,
                        minLength:{value:10,message:"min length is 10"}
                    })}/>
                    <div className='h-5 text-red-700 text-sm'>{errors.phonenumber&&<p className='text-xm'>*{errors.phonenumber.message}</p>}</div>
